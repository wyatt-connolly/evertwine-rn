...
export default function MessagesScreen({ navigation }: any) {
  ...
  const handleDeleteRoom = async (roomId: string, roomName: string) => {
    console.log("🗑️ [MessagesScreen] handleDeleteRoom called:", {
      roomId,
      roomName,
    });

    // Get room details to check if it's a meetup chat
    let roomData: MessageRoom | null = null;
    try {
      roomData = await SupabaseDataService.getMessageRoom(roomId);
      console.log("🗑️ [MessagesScreen] Room data fetched:", {
        roomId: roomData?.id,
        type: roomData?.type,
      ...
      console.error("❌ [MessagesScreen] Error fetching room data:", error);
    }

    const isMeetupChat = roomData?.type === "meetup";

    Alert.alert(
      "Delete Conversation",
      `Are you sure you want to delete "${roomName}"?${
        isMeetupChat
          ? "\n\nYou will also be removed from the associated meetup."
